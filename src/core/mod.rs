use std::{collections::HashMap, sync::{atomic::{AtomicBool, Ordering}, Arc, Mutex}, thread::JoinHandle};
...
impl<U> App<U> where U: Ui {
    pub fn new(preferences: Preferences, ui_link: U) -> Self {
        ...
        let thread = Some(worker::worker_thread(Arc::clone(&should_terminate), preferences.clone(), ui_link.clone(), metar_provider.clone(), vatsim_data_provider.clone()));
